...
export function ComplianceMap() {
  ...
  return (
    <div className="flex h-[calc(100vh-7rem)] gap-4">
      ...
      <div className="w-96 shrink-0 flex flex-col gap-3 overflow-hidden bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4">
        <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-200 dark:border-slate-700">
          <ShieldCheck className="w-5 h-5 text-indigo-500" />
          <h2 className="font-semibold text-slate-800 dark:text-white">ISO 9001:2015 Map</h2>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-2">
          {ISO_CLAUSES.map((clause) => {
            ...
            return (
              <div key={clause.id} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                <button
                  onClick={() => toggle(clause.id)}
                  className="w-full flex items-center gap-2 px-3 py-2 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors text-left"
                >
                  {expanded[clause.id] ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />}
                  <span className="font-bold text-slate-700 dark:text-slate-200 w-6">Cl {clause.id}</span>
                  <span className="flex-1 text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{clause.title}</span>
                  ...
                  {status === 'empty' && <Info className="w-4 h-4 text-slate-300 shrink-0" />}
                </button>
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-start gap-2 px-3 py-2 rounded text-left text-sm transition-colors ${
                            isSelected ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 text-slate-600 dark:text-slate-400'
                          }`}
                        >
                          <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 ${docs.length > 0 ? 'bg-green-500' : 'bg-red-400'}`} />
                          <span className="flex-1">{sub}</span>
                          {docs.length > 0 && <span className="text-xs bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">{docs.length} docs</span>}
                        </button>
                      );
                    })}
                  </div>
        ...
        {selectedSub ? (
          <>
            <div className="p-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
              ...
              <p className="text-sm text-slate-500 mt-1">Mapped Documents & Processes</p>
            </div>
            <div className="p-5 flex-1 overflow-y-auto">
              {getSubDocs(selectedSub).length > 0 ? (
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  {getSubDocs(selectedSub).map((doc) => (
                    <div key={doc.id} className="border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex gap-3 hover:border-indigo-300 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200 truncate" title={doc.tt || doc.no}>{doc.tt || doc.no}</h4>
                        <div className="text-xs text-slate-500 mt-1 flex items-center gap-2 flex-wrap">
                          <span className="font-medium bg-slate-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">{doc.no}</span>
                ...
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  ...
                  <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">Gap Identified</h4>
                  <p className="text-xs text-slate-500 mt-1 max-w-sm">No active documents are currently mapped to Clause {selectedSub}. You may need to create a new procedure or update an existing document's ISO clause mapping.</p>
                </div>
              )}
            </div>
          </>
