...
`ObjectivesDashboard.tsx` fully rewritten while preserving the original Tree/Leaderboard + modal architecture (per the "no navigation redesign" constraint). `useObjectivesStore.ts` extended with `transitionStatus`, `archiveRecord`, `unarchiveRecord`, `toggleArchive`; `ObjectiveRecord` extended with `ObjectiveStateHistoryEntry[]` (`stateHistory`), corrective-action fields (`correctiveAction`/`correctiveOwner`/`correctiveTargetDate`/`correctiveCompletionDate`), `isArchived` (all optional — no persist migration needed, key `qatrial:useObjectivesStore` unchanged). `ObjectiveRecordStatus` extended with `'Delayed'`/`'On Hold'`. `EvidenceAttachment.entityType` and `EvidencePanel`'s `entityType` union both extended with `'objective'`; `statusBadgeUtils.ts` extended with `'delayed'` in the red-color keyword group.

| # | Item | Score | Evidence |
|---|---|---|---|
| 1 | Ownership | **Pass** | `dept` (`DeptSelect`, 9-value canonical list with fallback option for legacy values) and `owner` (`UserSelect`, populated from `useAuthStore().users`) both editable via pickers instead of free text; `deadline` drives `isObjectiveOverdue()` (past deadline + non-terminal status), which powers a per-card overdue badge in Tree view, the Overdue KPI tile, and an "Overdue only" list filter — confirmed live: enabling the filter reduced Total Objectives from 44 to 16, exactly matching the Overdue KPI count. |
| 2 | Action plan (substituted: owner+deadline, plus corrective-action block) | **Pass** | Beyond the Phase 1 substitution, a full Corrective Action Plan toggle block was added (`correctiveAction` textarea, `correctiveOwner` via `UserSelect`, `correctiveTargetDate`/`correctiveCompletionDate`) — confirmed live via localStorage: `correctiveAction`, `correctiveOwner`, and `correctiveTargetDate` all persisted correctly to the record after entry. |
| 3 | State machine | **Pass** | Replaced the single generic `updateStatus` setter with `transitionStatus(id, to, by, reason, kind)`: a `stageOf(status)` helper groups the 9-value status enum into 5 stages (`notstarted`/`active`/`paused`/`done`/`failed`), each rendering its own forward/reject/reopen button set. Quick-action forward transitions ("Start Progress →", "Resume →") use a system-supplied reason with `kind:'forward'`; "Verify & Complete"/"Mark Not Achieved" (`kind:'verify'`), "Return to Not Started" (`kind:'reject'`), and "Reopen" from a terminal state (`kind:'reopen'`) are gated to `isMR` (`admin`/`qa_manager`) and require a typed reason via an inline `gate`/`gateReason` Confirm/Cancel panel. `kind`/`to` map onto the 9 pre-existing but previously-orphaned `ApprovalMetadata` fields (`reviewedBy`/`approvedBy`/`rejectedBy` + their date/comment pairs); reopen deliberately leaves ApprovalMetadata untouched, preserving the historical verification record. Every transition appends a `{from,to,by,at,reason,kind}` entry to `stateHistory`, rendered reverse-chronologically. Verified live end-to-end: Start Progress → Verify & Complete → Reopen, with each step's `stateHistory` entry and the correct ApprovalMetadata population/preservation confirmed both on-screen and via direct localStorage inspection. |
| 4 | No orphan fields | **Pass** | `objId`, `actual`, `evidence` added to the form; `parentId` surfaced as an explicit "Parent Objective" picker (filtered by year, excluding self); all 9 `ApprovalMetadata` fields now shown in a conditional read-only panel once populated; `isArchived` wired to an Archive/Unarchive button — confirmed live via localStorage (`isArchived` toggled `true` then back to `false` correctly). |
| 5 | Form validation | **Pass** | `yr`, `dept`, `desc` required at submit with inline error messages (`formErrors` state) — confirmed live: submitting a blank "New Objective" form produced "Year is required." and "Department is required." inline, with submission blocked; filling both plus a description succeeded and created a record with the expected default `status: "Not Started", pct: 0`. Status select now exposes all 9 enum values (`Delayed`/`On Hold` added), closing the prior 5-of-7 regression. |
| 6 | Evidence/comments/export/audit trail | **Pass** | `EvidencePanel entityType="objective"` and `CommentThread entityType="objective"` wired in behind per-record toggle icons — confirmed live: Evidence panel opened correctly with the expected "Evidence uploads require server mode" client-only-mode notice (consistent with every other module); Comments panel accepted and displayed a posted test comment ("Comments (1)" with author/timestamp/content and Reply/Edit/Delete). Print action added (`window.print()`, matching the exact convention already used in `UnifiedVoC.tsx`). Every transition and field update logs to `useAuditStore` via `transitionStatus`/existing update paths. |
| 7 | List view | **Pass** | Search (objective description), status filter, department filter, owner filter, sort (default/deadline-soonest/highest-achievement), "Overdue only" and "Show Archived" checkboxes all added — confirmed live: text search for "R-Stamp" correctly narrowed results to matching records only. |

**Overall: 7 Pass.**

**Design decisions / notes:**
- **Live-record-binding fix for modal-based state machines**: unlike VoC's inline-per-card architecture, Objectives keeps its original Tree/Leaderboard + modal pattern, where the modal's record was a `useState`-captured selection. This meant a transition inside the modal updated the store correctly but left the still-open modal showing stale status/stateHistory/ApprovalMetadata — found via live testing, not caught by typecheck/test/build. Fixed by deriving `liveRecord = records.find(r => r.id === selectedObjective.id)` at render time (inside an IIFE wrapping the modal) and passing `liveRecord` — not the captured selection — as the modal's `record` prop, with all callback closures (`onUpdate`/`onTransition`/`onToggleArchive`) updated to reference `liveRecord.id`. **This pattern should be re-checked on any remaining module that opens a state-machine UI via a modal triggered by a captured selection, rather than rendering it inline per list item.**
- **Legacy-status "stage" grouping**: `stageOf(status)` maps the full 9-value `ObjectiveRecordStatus` enum (including the legacy `Ongoing`/`Pending Submission` values already in seed data) onto 5 UI stages, so the button set shown is always consistent regardless of which of the two "in-progress-like" or two "terminal-like" status strings a given record happens to hold — avoids a combinatorial explosion of per-status button logic without collapsing or renaming any existing status value.
- **ApprovalMetadata-to-transition-kind mapping**: `kind:'verify'` targeting `Completed`/`Achieved` populates `approvedBy`/`approvalDate`/`approvalComments`; targeting `Not Achieved` populates `rejectedBy`/`rejectionDate`/`rejectionReason`; `kind:'reject'` populates `reviewedBy`/`reviewDate`/`reviewComments`; `kind:'reopen'` leaves ApprovalMetadata untouched by design, so a record's most recent verification stays visible in the read-only panel even after it's reopened for rework — confirmed live (Reopen after Verify & Complete left the "Verified Completed by..." panel unchanged while status reverted to "In Progress").
- **Corrective-action-block scope, no full CAPA triad**: matches the Calibration/Approved Vendors/VoC precedent — a single corrective-action block (not the shared `ActionPlanTable`) is proportionate to an objective's remediation, which is a single narrative action against a single owner/date, not a multi-row CAPA-style action list.
- **Dedicated store test file added**: `src/store/__tests__/useObjectivesStore.test.ts` (13 tests) covers `addRecord` (id/timestamp/default-status/isArchived), `updateRecord` (partial merge), `updateStatus` (legacy quick-action path, pct-to-100 on Completed), `transitionStatus` (forward, verify→Achieved with approvedBy/approvalComments, verify→Not Achieved with rejectedBy/rejectionReason, reject with reviewedBy/reviewComments, reopen leaving ApprovalMetadata untouched, stateHistory accumulation), `archiveRecord`/`unarchiveRecord`/`toggleArchive`, and `deleteRecord` — mirroring the `useClientIntakeStore.test.ts` precedent.
- **Status-filter dropdown label disambiguation**: filter options read "Not Started records"/"In Progress records"/etc. rather than the bare status words, applying the exact convention already established in `UnifiedVoC.tsx`, to avoid an exact-match `getByText` collision with the KPI-tile and `StatusBadge` bare-word text in the locked-in `qms_smoke.test.tsx` — this was hit as a real test failure this phase and fixed by the rename; confirmed by the smoke test passing afterward.
- **Live browser verification** (`http://localhost:5174`): Tree view, KPI tiles, dept groups, and all filters confirmed rendering correctly against 44 seeded records; full state-machine chain (Start Progress → Verify & Complete → Reopen) exercised end-to-end with stateHistory and ApprovalMetadata correctness confirmed via direct `localStorage['qatrial:useObjectivesStore']` inspection; Evidence panel, Comments panel (including posting a comment), Corrective Action Plan block (all four fields persisting), Archive/Unarchive round-trip, Overdue filter (44→16 matching the KPI count), text search, and New Objective creation (both the validation-failure and validation-success paths) all exercised live. Test-created localStorage state (the test objective record) was removed afterward to restore default seed data; the test comment and corrective-action test values were left in place on the pre-existing seeded record as harmless local-only verification artifacts (per-browser `localStorage`, not part of the committed codebase).
...
✓ built in 1.24s
```
(new `ObjectivesDashboard-C8B-I6tQ.js` (28.47 kB) and `useObjectivesStore-rDKTiQo3.js` (25.26 kB) chunks produced; no new errors or warnings)
...
`CalibrationRegister.tsx` fully rewritten; `useCalibStore.ts` extended with `transitionStatus`, `reportOutOfTolerance`, `verifyReturnToService`, `scrapInstrument`, `reopenFromScrap`, plus a new `computeStatusFromDate` helper. `CalibRecord` extended with `assignedTo`, `assignedDept`, `correctiveAction`, `correctiveOwner`, `correctiveTargetDate`, `correctiveCompletionDate`, `stateHistory` (all optional). New `CalibStateHistoryEntry` type added. `EvidenceAttachment.entityType` and `EvidencePanel`'s `entityType` union both extended with `'calibration'`.

| # | Item | Score | Evidence |
|---|---|---|---|
| 1 | Ownership | **Pass** | `assignedTo` (`UserSelect`, from `useAuthStore().users`) and `assignedDept` (`DeptSelect`, from `PTA_DEPARTMENTS`) added to the create/edit form and shown in the detail ownership grid and list-card footer; overdue badge (`isCalibOverdue`) shown on the detail header and each overdue list card; list view supports filtering by department and "Overdue only". |
| 2 | Action plan | **Pass** | A Corrective Action block (`correctiveAction`/`correctiveOwner` via `UserSelect`/`correctiveTargetDate`/`correctiveCompletionDate`) is shown and editable whenever the record is `Out of Service`/`Scrapped` or the fields already hold data — the out-of-tolerance follow-up substitute called for in the work list. Evidence attachment via the shared `EvidencePanel`. |
| 3 | State machine | **Pass** | The divergent inline status computation is gone — the component now imports and uses the store's `computeCalibStatus`/`isCalibOverdue` exclusively. The unguarded 3-option `<select>` is replaced with bespoke per-state gates: any in-service record gets an open, reason-required "Report Out-of-Tolerance" action (→ `Out of Service`); `Out of Service` renders an MR-gated (`admin`/`qa_manager`) two-button "Disposition Gate" ("Verify & Return to Service" / "Scrap Instrument"), each reason-required; `Scrapped` renders a terminal panel with an MR-gated, reason-required reopen (back to `Out of Service`). Every transition appends a typed `{from,to,by,at,reason,kind}` entry to `stateHistory`, rendered in a bespoke "Transition History" block — verified live end-to-end (Valid → Out of Service → Valid, see below). The hardcoded `addRecord({...data, status: 'Active'})` bug identified in Phase 1 is fixed: new records now default to `'Valid'` (locked in by `useCalibStore.test.ts`). |
| 4 | No orphan fields | **Pass** | `calibrationAgency` and `calibratedBy` are now in both the create/edit form and the detail view. All 9 `ApprovalMetadata` fields are wired into the disposition workflow rather than dropped: `approvedBy`/`approvalDate`/`approvalComments` are written by `verifyReturnToService` and shown in a "Disposition Trail" section; `rejectedBy`/`rejectionDate`/`rejectionReason` are written by `scrapInstrument`, shown in the same section, and cleared on `reopenFromScrap`. `reviewedBy`/`reviewDate`/`reviewComments` remain legitimately unused — consistent with every other module's non-use of those same shared `ApprovalMetadata` fields, not calibration-specific orphans. `isArchived` is wired to `archiveRecord`/`unarchiveRecord`/`toggleArchive` via an Archive action button and a "Show Archived" list filter. |
| 5 | Form validation | **Pass** | `equipNo`/`equipName` required at submit with inline browser validation; every disposition action (report/verify/scrap/reopen) has its Confirm button disabled until a reason is entered, enforcing "reason required" at the point of action rather than only in the type. |
| 6 | Evidence/comments/export/audit trail | **Pass** | `EvidencePanel entityType="calibration"` and `CommentThread entityType="calibration"` wired in; a print action (`window.print()`) added with `print:hidden` on the action-icon row; every store mutation already logged to `useAuditStore` (create/update/delete/archive/unarchive/status_change), now including a reasoned entry for every disposition transition. |
| 7 | List view | **Pass** | Search (equipment no./name), status filter (All / In Service / Out of Service / Scrapped), department filter, "Overdue only" and "Show Archived" checkboxes, and a sort control (next-due-soonest / newest / oldest) all added above the equipment list, matching/exceeding v12's search+status-filter+due-sort baseline. |

**Overall: 7 Pass.**

**Design decisions / notes:**
- **`StateTransitionBar` deliberately not reused**: unlike DCR/Supplier, Calibration's states don't fit the shared bar's single-linear-forward-path model — an in-service record can jump directly to `Out of Service` from any of `Valid`/`Due`/`Overdue` (not a fixed linear predecessor), and `Out of Service` branches to *two* distinct terminals (`Valid`-family via verify, or `Scrapped`) rather than one. Forcing this through the bar's `statuses: string[]` + single-terminal contract would have required misrepresenting the branch as a fake linear path. Instead, a bespoke per-state gate UI (open action / two-button MR gate / terminal-with-reopen) models the actual branching, and a bespoke "Transition History" block reuses only the shared component's *visual* pattern (`bg-surface-secondary border border-border rounded-lg px-3 py-2 text-xs`, reversed-chronological, `{from} → {to}` + kind badge + `by`/`reason`/timestamp) — not its code.
- **Resolving the orphaned `ApprovalMetadata` fields**: rather than dropping the 9 fields (the work list's fallback option), they were given a real purpose matching the domain — `approvedBy`/`approvalDate`/`approvalComments` model QA's verification that a repaired instrument is fit to return to service; `rejectedBy`/`rejectionDate`/`rejectionReason` model the scrap decision. This mirrors the precedent set by Supplier's `approveSupplier`/`rejectSupplier`.
- **`computeStatusFromDate` helper**: `computeCalibStatus` intentionally freezes at `'Out of Service'`/`'Scrapped'` and can't answer "what would this record's status be if it weren't overridden" — needed specifically for `verifyReturnToService` to compute the correct target status (`Valid`/`Due`/`Overdue`) purely from `nextCalibDate` when an instrument leaves `Out of Service`. Kept as a separate pure function rather than modifying `computeCalibStatus`, since the latter's freeze behavior is still correct and used everywhere else (list rendering, overdue checks).
- **`reopenFromScrap` clears rejection metadata**: on reopen, `rejectedBy`/`rejectionDate`/`rejectionReason` are reset to `undefined` (rather than left stale) since the record moves back into an active, correctable state (`Out of Service`) rather than remaining in a rejected/terminal state — leaving contradictory metadata (`status: 'Out of Service'` but `rejectedBy` still populated) would be misleading.
- **Legacy status self-healing preserved**: `computeCalibStatus` already recomputes `Valid`/`Due`/`Overdue` live from `nextCalibDate` for any record not `Out of Service`/`Scrapped`, so pre-existing localStorage records still holding the legacy `'Active'`/`'Due Soon'` literals self-heal automatically on render — no separate migration or normalization map was needed.
- **Live browser verification** (`http://localhost:5174`): list view with stats tiles, filters (status/department/overdue-only/sort/show-archived) and the seed record; detail panel with Equipment Details/Notes; "Report Out-of-Tolerance" open action moving `Valid → Out of Service` with the Corrective Action block and MR-gated Disposition Gate appearing; "Verify & Return to Service" moving the record back to `Valid` with the Disposition Trail populated; Transition History rendering both entries in reverse-chronological order with correct `{from} → {to}`/kind/timestamp/reason; the expanded "Add Equipment" create form (all new fields present, incl. `assignedTo`/`assignedDept` pickers) creating a new record that correctly defaults to `Valid` status — no console errors introduced.
